...
public class PencilPipeline implements LifeCycle {
  ...
  private final String FRAGMENT_SRC = """
      ...
      void main() {
          if (inProperties.x == 1.0f) {
            //outFragColor = vec4(inColor.rgb, texture(textureSampler, vec3(inTexture,inProperties.y)).r);
            float originAlpha = texture(textureSampler, vec3(inTexture,inProperties.y)).r;
  ...
  public PencilPipeline(Renderer renderer, VkRenderPass pass, Pencil pencil) {
  ...
  private void setupDescriptorset() throws ThemisException {
    this.pencilDescriptorSet = new PencilDescriptorSet(this.renderer, this.pencil);
  ...
  private void updateBuffers() throws ThemisException {
    ...
    if (this.drawCommandVertexBuffer == null || this.drawCommandVertexBuffer.getRequestedSize() < dataSize) {
      ...
      this.drawCommandVertexBuffer = new VkBuffer(this.renderer.getDevice(), this.renderer.getMemoryAllocator(), decriptor);
    ...
    if (this.drawCommandIndiceBuffer == null || this.drawCommandIndiceBuffer.getRequestedSize() < indiceSize) {
      ...
      this.drawCommandIndiceBuffer = new VkBuffer(this.renderer.getDevice(), this.renderer.getMemoryAllocator(), decriptorIndices);
  ...
  private void setupPipeline() throws ThemisException {
    ...
    VkShaderProgramStage vertexShader = new VkShaderProgramStage(VK_SHADER_STAGE_VERTEX_BIT, VkShaderSourceCompiler.compileShader(VERTEX_SRC, Shaderc.shaderc_glsl_vertex_shader));

    VkShaderProgramStage fragmentShader = new VkShaderProgramStage(VK_SHADER_STAGE_FRAGMENT_BIT, VkShaderSourceCompiler.compileShader(FRAGMENT_SRC, Shaderc.shaderc_glsl_fragment_shader));

    this.shaderProgram = new VkShaderProgram(this.renderer.getDevice(), vertexShader, fragmentShader);
    this.shaderProgram.setup();

    this.pipelineLayout = new VkPipelineLayout(this.renderer.getDevice(), new VkPushConstantRange[0], this.pencilDescriptorSet.getDescriptorSetLayout());
    this.pipelineLayout.setup();

    try (MemoryStack stack = MemoryStack.stackPush()) {

      VkVertexInputStateDescriptor descriptor = new VkVertexInputStateDescriptor(VK_VERTEX_INPUT_RATE_VERTEX).attribute(VK_FORMAT_R32G32_SFLOAT, MemorySizeUtils.VEC2F) // 2D position
      ...
      this.pipeline = new VkPipeline(this.renderer.getDevice(), new VkPipelineDescriptor(this.pass, 0, true, 1, false, 1, 1, 1), this.shaderProgram, this.pipelineLayout, inputState);
      this.pipeline.setup();
    }
  }
}
