...
public class Pencil {
    ...
    public Pencil drawRect(float x, float y, float width, float height, float r, float g, float b) {
        ...
        setData(data, getDataSize() + COMPONENT_COUNT * 2, x + width, y + height, 1.0f, 1.0f, r, g, b, false);
    ...
    private Pencil drawRect(float x, float y, float width, float height, float uMin, float vMin, float uMax, float vMax) {

        int startIndiceOffset = getDataSize() / COMPONENT_COUNT;

        float [] data = extendData(4);
        setData(data, getDataSize(), x, y, uMin, vMin, 0.0f, 0.0f, 0.0f, true);
        setData(data, getDataSize() + COMPONENT_COUNT, x + width, y, uMax, vMin, 0.0f, 0.0f, 0.0f, true);
        setData(data, getDataSize() + COMPONENT_COUNT * 2, x + width, y + height, uMax, vMax, 0.0f, 0.0f, 0.0f, true);
        setData(data, getDataSize() + COMPONENT_COUNT * 3, x, y + height, uMin, vMax, 0.0f, 0.0f, 0.0f, true);
        this.data = data;
    ...
    private void setData(float[] data, int idx, float x, float y, float u, float v, float r, float g, float b, boolean useTexture) {
