...
public record Vertex(
    Vector3f position, Vector3f normale, Vector2f texture, Vector3f tangent, Vector3f bitangent) {

  public static final int COMPONENTS = 3 + 3 + 2 + 3 + 3;

  public static Vertex of(Vertex vertex) {
    return new Vertex(
        vertex.position, vertex.normale, vertex.texture, vertex.tangent, vertex.bitangent);
  }

  public static Vertex of(
      Vector3f position, Vector3f normale, Vector2f texture, Vector3f tanget, Vector3f bitangent) {
    return new Vertex(position, normale, texture, tanget, bitangent);
  }

  public static Vertex of(float x, float y, float z) {
    return new Vertex(
        new Vector3f(x, y, z), new Vector3f(), new Vector2f(), new Vector3f(), new Vector3f());
  }

  public static Vertex of(float x, float y, float z, float u, float v) {
    return new Vertex(
        new Vector3f(x, y, z), new Vector3f(), new Vector2f(u, v), new Vector3f(), new Vector3f());
  }
