...
public class VkVertexInputState {
  ...
  private VkVertexInputAttributeDescription.Buffer createAttributeDescription(MemoryStack stack) {
    ...
    VkVertexInputAttributeDescription.Buffer attributeDescription =
        VkVertexInputAttributeDescription.calloc(count, stack);
  ...
  private VkVertexInputBindingDescription.Buffer createBindingDescription(MemoryStack stack) {
    ...
    VkVertexInputBindingDescription.Buffer bindingDescription =
        VkVertexInputBindingDescription.calloc(this.descriptor.length, stack);
    ...
    for (VkVertexInputStateDescriptor descriptor : this.descriptor) {
      if (descriptor.length() > 0) {
        bindingDescription
            .get(binding)
            .binding(binding)
            .stride(descriptor.length())
            .inputRate(descriptor.getInputRate());
      }
      binding++;
    }
