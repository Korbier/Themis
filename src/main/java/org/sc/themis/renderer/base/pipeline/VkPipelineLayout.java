...
public class VkPipelineLayout extends VulkanObject {
  ...
  public void setup() throws ThemisException {
    try (MemoryStack stack = MemoryStack.stackPush()) {
      org.lwjgl.vulkan.VkPushConstantRange.Buffer pushConstantRanges =
          createPushConstantRanges(stack);
      LongBuffer layouts = createLayouts(stack);
      VkPipelineLayoutCreateInfo layoutCreateInfo =
          createLayoutCreateInfo(stack, pushConstantRanges, layouts);
  ...
  private VkPipelineLayoutCreateInfo createLayoutCreateInfo(
      MemoryStack stack,
      org.lwjgl.vulkan.VkPushConstantRange.Buffer pushConstantRanges,
      LongBuffer layouts) {
  ...
  private org.lwjgl.vulkan.VkPushConstantRange.Buffer createPushConstantRanges(MemoryStack stack) {
    ...
    if (this.constantRanges.length > 0) {
      ...
      org.lwjgl.vulkan.VkPushConstantRange.Buffer pushConstantRanges =
          org.lwjgl.vulkan.VkPushConstantRange.calloc(this.constantRanges.length, stack);
