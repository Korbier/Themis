...
public class Frame {
  ...
  <T extends VulkanObject> T create(FrameKey<T> key, SupplierWithException<T> supplier)
      throws ThemisException {
    return put(key, supplier.get());
  }

  <T extends VulkanObject> T create(
      FrameKey<T> key, int frame, FunctionWithException<Integer, T> function)
      throws ThemisException {
    return put(key, function.apply(frame));
  }

  <T extends VulkanObject> T update(FrameKey<T> key, ConsumerWithException<T> consumer)
      throws ThemisException {
  ...
  <T extends VulkanObject> T update(
