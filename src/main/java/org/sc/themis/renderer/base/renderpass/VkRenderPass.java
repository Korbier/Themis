...
public class VkRenderPass extends VulkanObject {
  ...
  private org.lwjgl.vulkan.VkSubpassDependency.Buffer createSubpassDependencies(MemoryStack stack) {
    ...
    org.lwjgl.vulkan.VkSubpassDependency.Buffer subpassDependencies =
        org.lwjgl.vulkan.VkSubpassDependency.calloc(dependencies.size(), stack);
    ...
    for (VkSubpassDependency dependency : dependencies) {
      ...
      subpassDependencies
          ...
          .srcStageMask(
              dependency
                  .srcStageMask()) // https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkPipelineStageFlagBits.html
  ...
  private VkAttachmentDescription.Buffer createAttachmentDescription(MemoryStack stack) {
    ...
    VkAttachmentDescription.Buffer attachmentsDesc =
        VkAttachmentDescription.calloc(layout.size(), stack);
  ...
  private VkSubpassDescription.Buffer createSubpassDescription(MemoryStack stack) {
    ...
    VkSubpassDescription.Buffer subpassDesc =
        VkSubpassDescription.calloc(this.descriptor.getSubpasses().size(), stack);
  ...
  private VkAttachmentReference.Buffer createResolveReferences(
      MemoryStack stack, VkSubpass subpass) {
  ...
  private VkAttachmentReference.Buffer createReferences(
      MemoryStack stack, Map<Integer, Integer> layouts) {

    if (layouts.isEmpty()) {
      return null;
    }

    VkAttachmentReference.Buffer inputReferences =
        VkAttachmentReference.calloc(layouts.size(), stack);
