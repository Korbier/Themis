...
public class VkFrameBufferAttachments extends VulkanObject {
  ...
  public long getImageCount() {
    return this.attachments.values().stream()
        .filter(a -> a.getType() != VkFrameBufferAttachment.VkFrameBufferAttachmentType.RAW)
        .count();
  }

  public boolean hasDepthAttachment() {
    return this.attachments.values().stream()
        .anyMatch(a -> a.getType() == VkFrameBufferAttachment.VkFrameBufferAttachmentType.DEPTH);
  }
