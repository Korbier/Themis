...
public class Font {
  ...
  private void createBitmapBuffer(FT_Face face) {

    int bitmapPadding = 32;
    int bitmapCols = 16;
    int imageWidth  = (64 + bitmapPadding) * bitmapCols;//(this.size + this.bitmapPadding) * this.bitmapCols;
    int bitmapRows = 16;
    int imageHeight = (64 + bitmapPadding) * bitmapRows;//(this.size + this.bitmapPadding) * this.bitmapRows;

    this.characters = new HashMap<>();
    ByteBuffer bitmapBuffer = BufferUtils.createByteBuffer(imageWidth * imageHeight + bitmapPadding);
    ...
    for (int i = 0; i < nbChar; i++) {
      ...
      if (charBitmapBuffer != null) {
        ...
        for (int j = 0; j < charHeight; j++) {
          ...
          bitmapBuffer.put(x + ((y + j) * imageWidth), charBitmapBuffer, srcStart, charWidth);
        }
    ...
    this.texture = Image.of(bitmapBuffer, imageWidth, imageHeight);
