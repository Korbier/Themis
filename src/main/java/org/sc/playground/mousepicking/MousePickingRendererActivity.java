...
/**
 * 1 pass, 3 subpass : subpass 0 : draw instances identifiers subpass 1 : pickup fragment at mouse
 * position subpass 2 : draw instances with selected instance highlight
 */
public class MousePickingRendererActivity extends RendererActivity {

  private static final String FB_ATTACHMENT_PRESENTATION = "framebuffer.attachment.presentation";
  private static final String FB_ATTACHMENT_DEPTH = "framebuffer.attachment.depth";
  private static final String FB_ATTACHMENT_IDENTIFIER = "framebuffer.attachment.identifier";

  private static final String SHADER_0_VERTEX_SOURCE = "src/main/resources/playground/mousepicking/0_vertex_shader.glsl";
  private static final String SHADER_0_VERTEX_COMPILED = "target/playground/mousepicking/0_vertex_shader.spirv";
  private static final String SHADER_0_FRAGMENT_SOURCE = "src/main/resources/playground/mousepicking/0_fragment_shader.glsl";
  private static final String SHADER_0_FRAGMENT_COMPILED = "target/playground/mousepicking/0_fragment_shader.spirv";

  private static final String SHADER_1_VERTEX_SOURCE = "src/main/resources/playground/mousepicking/1_vertex_shader.glsl";
  private static final String SHADER_1_VERTEX_COMPILED = "target/playground/mousepicking/1_vertex_shader.spirv";
  private static final String SHADER_1_FRAGMENT_SOURCE = "src/main/resources/playground/mousepicking/1_fragment_shader.glsl";
  private static final String SHADER_1_FRAGMENT_COMPILED = "target/playground/mousepicking/1_fragment_shader.spirv";

  private static final String SHADER_2_VERTEX_SOURCE = "src/main/resources/playground/mousepicking/2_vertex_shader.glsl";
  private static final String SHADER_2_VERTEX_COMPILED = "target/playground/mousepicking/2_vertex_shader.spirv";
  private static final String SHADER_2_FRAGMENT_SOURCE = "src/main/resources/playground/mousepicking/2_fragment_shader.glsl";
  private static final String SHADER_2_FRAGMENT_COMPILED = "target/playground/mousepicking/2_fragment_shader.spirv";
  ...
  private void setupDescriptorSets() throws ThemisException {
    ...
    this.mousePickingDescriptorSet = new MousePickingDescriptorSet(this.renderer);
    this.mousePickingDescriptorSet.setup();

    this.subpass1DescriptorSet = new FramebufferAttachmentDescriptorSet(this.renderer, this.frameBufferAttachments.get(FB_ATTACHMENT_IDENTIFIER));
    this.subpass1DescriptorSet.setup();
  }

  private void setupFramebuffers() throws ThemisException {
    this.renderer.getFramesInFlight().create(FK_FRAMEBUFFER, (frame) -> {
      VkFrameBufferDescriptor descriptor = new VkFrameBufferDescriptor(this.renderer.getExtent(), this.renderPass.getHandle(), this.renderer.getImageView(frame).getHandle(), this.frameBufferAttachments.get(FB_ATTACHMENT_DEPTH).getView().getHandle(), this.frameBufferAttachments.get(FB_ATTACHMENT_IDENTIFIER).getView().getHandle());
      return new VkFrameBuffer(this.renderer.getDevice(), descriptor);
    });
  }

  private void setupFramebufferAttachments() throws ThemisException {
    this.frameBufferAttachments = new VkFrameBufferAttachments(renderer.getDevice(), this.renderer.getExtent());
  ...
  private void setupRenderPass() throws ThemisException {
    ...
    this.renderPass = new VkRenderPass(renderer.getDevice(), descriptor);
    this.renderPass.setup();
  }

  private void setupFence() throws ThemisException {
    this.renderer.getFramesInFlight().create(FK_FENCE, () -> new VkFence(this.renderer.getDevice(), false));
  }

  private void setupCommand() throws ThemisException {
    this.renderer.getFramesInFlight().create(FK_COMMAND, () -> this.renderer.createGraphicCommand(true));
  }

  private VkRenderPassDescriptor createSubPassDescriptor(VkDevice device) {

    VkRenderPassLayout layout = new VkRenderPassLayout().add(0, this.renderer.getImageFormat(), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_STORE_OP_STORE, VK_ATTACHMENT_LOAD_OP_DONT_CARE, VK_ATTACHMENT_STORE_OP_DONT_CARE).add(1, this.frameBufferAttachments.get(FB_ATTACHMENT_DEPTH).getFormat(), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, VK_ATTACHMENT_LOAD_OP_DONT_CARE, VK_ATTACHMENT_STORE_OP_DONT_CARE, VK_ATTACHMENT_LOAD_OP_DONT_CARE, VK_ATTACHMENT_STORE_OP_DONT_CARE).add(2, this.frameBufferAttachments.get(FB_ATTACHMENT_IDENTIFIER).getFormat(), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_STORE_OP_STORE, VK_ATTACHMENT_LOAD_OP_DONT_CARE, VK_ATTACHMENT_STORE_OP_DONT_CARE);
  ...
  private void setupShaderProgram0() throws ThemisException {
    try {
      ...
      this.shaderProgram0 = new VkShaderProgram(renderer.getDevice(), vertexStage, fragmentStage);
  ...
  private void setupPipeline0() throws ThemisException {
    ...
    this.pipelineLayout0 = new VkPipelineLayout(this.renderer.getDevice(), new VkPushConstantRange[]{new VkPushConstantRange(VK_SHADER_STAGE_VERTEX_BIT, 0, MemorySizeUtils.PUSHCONSTANT), new VkPushConstantRange(VK_SHADER_STAGE_FRAGMENT_BIT, MemorySizeUtils.PUSHCONSTANT, MemorySizeUtils.PUSHCONSTANT)}, this.sceneDescriptorSet.getDescriptorSetLayout());
    this.pipelineLayout0.setup();

    try (MemoryStack stack = MemoryStack.stackPush()) {

      VkVertexInputStateDescriptor descriptor = new VkVertexInputStateDescriptor(VK_VERTEX_INPUT_RATE_VERTEX).attribute(VK_FORMAT_R32G32B32_SFLOAT, MemorySizeUtils.VEC3F) // Position
      ...
      this.pipeline0 = new VkPipeline(this.renderer.getDevice(), new VkPipelineDescriptor(this.renderPass, 0, false, 1, true, 1, 1, 1), this.shaderProgram0, this.pipelineLayout0, inputState);
  ...
  private void setupShaderProgram1() throws ThemisException {
    try {
      ...
      this.shaderProgram1 = new VkShaderProgram(renderer.getDevice(), vertexStage, fragmentStage);
  ...
  private void setupPipeline1() throws ThemisException {
    ...
    this.pipelineLayout1 = new VkPipelineLayout(this.renderer.getDevice(), new VkPushConstantRange[0], this.subpass1DescriptorSet.getDescriptorSetLayout(), this.mousePickingDescriptorSet.getDescriptorSetLayout());
    ...
    try (MemoryStack stack = MemoryStack.stackPush()) {
      ...
      this.pipeline1 = new VkPipeline(this.renderer.getDevice(), new VkPipelineDescriptor(this.renderPass, 1, false, 1, false, 1, 1, 1), this.shaderProgram1, this.pipelineLayout1, inputState);
  ...
  private void setupShaderProgram2() throws ThemisException {
    try {
      ...
      this.shaderProgram2 = new VkShaderProgram(renderer.getDevice(), vertexStage, fragmentStage);
  ...
  private void setupPipeline2() throws ThemisException {
    ...
    this.pipelineLayout2 = new VkPipelineLayout(this.renderer.getDevice(), new VkPushConstantRange[]{new VkPushConstantRange(VK_SHADER_STAGE_VERTEX_BIT, 0, MemorySizeUtils.PUSHCONSTANT), new VkPushConstantRange(VK_SHADER_STAGE_FRAGMENT_BIT, MemorySizeUtils.PUSHCONSTANT, MemorySizeUtils.PUSHCONSTANT)}, this.sceneDescriptorSet.getDescriptorSetLayout(), this.mousePickingDescriptorSet.getDescriptorSetLayout());
    ...
    try (MemoryStack stack = MemoryStack.stackPush()) {
      ...
      this.pipeline2 = new VkPipeline(this.renderer.getDevice(), new VkPipelineDescriptor(this.renderPass, 2, false, 1, true, 1, 1, 1), this.shaderProgram2, this.pipelineLayout2, inputState);
